...
$(document).ready(async function () {
  ...
  require(["vs/editor/editor.main"], function (ignorable) {
    ...
    layout.registerComponent("chat", function (container, state) {
      ...
      chatContainer.className = "chat-container h-full flex flex-col bg-[#1e1e1e] border border-[#3e3e42] rounded-lg shadow-md";
    
      chatContainer.innerHTML = `
        <!-- Chat Header -->
        <div class="chat-header flex flex-col gap-3 bg-[#252526] border-b border-[#3e3e42] p-4 text-gray-300">
          <!-- Title Line -->
          <div class="flex items-center gap-3">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" d="M8.625 9.75a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H8.25m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0H12m4.125 0a.375.375 0 1 1-.75 0 .375.375 0 0 1 .75 0Zm0 0h-.375m-13.5 3.01c0 1.6 1.123 2.994 2.707 3.227 1.087.16 2.185.283 3.293.369V21l4.184-4.183a1.14 1.14 0 0 1 .778-.332 48.294 48.294 0 0 0 5.83-.498c1.585-.233 2.708-1.626 2.708-3.228V6.741c0-1.602-1.123-2.995-2.707-3.228A48.394 48.394 0 0 0 12 3c-2.392 0-4.744.175-7.043.513C3.373 3.746 2.25 5.14 2.25 6.741v6.018Z" />
            </svg>
            <span class="font-semibold">Code Assistant</span>
          </div>
        ...
        <!-- Messages -->
        <div class="messages flex-1 overflow-y-auto p-4 space-y-4 bg-[#1e1e1e]"></div>
    
        <!-- Input Box -->
        <div class="chat-input-container border-t border-[#3e3e42] p-4 bg-[#252526]">
          <div class="chat-input-wrapper flex gap-2 items-center">
            <textarea autofocus class="chat-input flex-1 bg-[#1e1e1e] text-gray-300 rounded-lg border-[#3e3e42] p-3 focus:outline-none focus:border-blue-500 resize-none h-12" placeholder="Type your message..."></textarea>
            <button type="button" class="send-btn bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 transition-colors">
              <span>Send</span>
      ...
      function addUserMessage(message) {
        const messageHTML = `
          <div class="flex justify-end">
            <div class="message bg-blue-500 text-white rounded-xl px-4 py-2 max-w-[80%]">
              <div>${message}</div>
      ...
      function addAssistantMessage(message) {
        const messageHTML = `
          <div class="flex justify-start">
            <div class="message bg-[#2a2a2a] text-gray-300 rounded-xl px-4 py-2 max-w-[80%]">
              <div>${message}</div>
              <div class="text-xs text-gray-400 mt-1">${formatTimestamp()}</div>
            </div>
          </div>
        `;
        messagesEl.insertAdjacentHTML("beforeend", messageHTML);
      ...
      function addTypingIndicator() {
        const indicatorHTML = `
          <div id="typing-indicator" class="flex justify-start">
            <div class="message bg-[#2a2a2a] text-gray-300 rounded-xl px-4 py-2">
      ...
      async function sendMessage() {
        ...
        try {
              ...
              'messages': [
                {
                  ...
                  ${codeContext.stdout ? `Input:\n${codeContext.stdout}\n` : ''}
  
                  Privude clear, consise and accurate responses about the code.
                  If suggesting code changes, explain the reasonong and the logic and ensure they follow the programmong languages coding best practices.
                  `,
                },
                {
                  role: 'user',
          ...
          console.log('Error', e)
          removeTypingIndicator()
          addAssistantMessage("Sorry, there was an error processing your request, Please make sure you have set up your OpenRouter key correctly !")
        }
